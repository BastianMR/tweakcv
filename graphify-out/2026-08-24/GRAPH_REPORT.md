...
## Corpus Check
- 118 files · ~42,464 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 780 nodes · 1305 edges · 99 communities (45 shown, 54 thin omitted)
...
5. `dataDir()` - 17 edges
6. `Quickstart: Validación end-to-end TweakCV MVP` - 17 edges
7. `requireActiveProfile()` - 16 edges
8. `ApiError` - 15 edges
9. `registerCvRoutes()` - 15 edges
10. `AGENTS.md Agent Instructions` - 14 edges

## Surprising Connections (you probably didn't know these)
- `clientWith()` --calls--> `createApp()`  [EXTRACTED]
  tests/unit/errors.test.ts → src/server/app.ts
- `uploadedDiploma()` --calls--> `runExtraction()`  [EXTRACTED]
  tests/integration/us2-documents.test.ts → src/server/documents/extraction.ts
...
### Community 0 - "db/index.ts"
Cohesion: 0.06
Nodes (28): AiClient, AiClientOptions, AiConfig, AiError, AiErrorCode, CompleteJsonInput, PRESET_BASE_URLS, resolveConfig() (+20 more)
...
### Community 2 - "App.tsx"
Cohesion: 0.06
Nodes (55): api, ApiClientError, ApiErrorShape, CvFull, CvSummary, useCvPreview(), useCvs(), useDeleteCv() (+47 more)
...
Cohesion: 0.04
Nodes (48): concurrently, eslint, @eslint/js, allowScripts, better-sqlite3@13.0.3, esbuild@0.28.2, puppeteer@25.8.0, description (+40 more)

### Community 5 - "dependencies"
Cohesion: 0.07
Nodes (29): ajv, better-sqlite3, handlebars, hono, @hono/node-server, @jsonresume/schema, openai, dependencies (+21 more)
...
### Community 8 - "render.ts"
Cohesion: 0.17
Nodes (18): SnapshotSkill, TailorCvOutput, tailorCvOutputSchema, compiled, contactLine(), flattenSections(), FlatView, renderHtml() (+10 more)
...
Cohesion: 0.05
Nodes (88): createAiClient(), mockExtractFor(), MOCK_PARSE, ParsePostingOutput, parsePostingOutputSchema, mockTailorFromSnapshot(), createApp(), dataDir() (+80 more)
...
### Community 93 - "api/postings.ts"
Cohesion: 0.35
Nodes (9): ParsedPosting, PostingRow, useCreatePosting(), useDeletePosting(), useInvalidatePostings(), usePostings(), useUpdatePosting(), ParsedEditor() (+1 more)
...
## Knowledge Gaps
- **283 isolated node(s):** `name`, `version`, `description`, `type`, `license` (+278 more)
...
- **Why does `api` connect `App.tsx` to `api/documents.ts`, `api/postings.ts`?**
  _High betweenness centrality (0.020) - this node is a cross-community bridge._
- **Why does `getDb()` connect `collections.ts` to `db/index.ts`?**
  _High betweenness centrality (0.019) - this node is a cross-community bridge._
- **Why does `ApiError` connect `collections.ts` to `db/index.ts`?**
